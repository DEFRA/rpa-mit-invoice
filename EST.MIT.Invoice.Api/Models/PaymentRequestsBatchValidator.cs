...
public class PaymentRequestsBatchValidator : AbstractValidator<PaymentRequestsBatch>
{
    ...
    public PaymentRequestsBatchValidator(IReferenceDataApi referenceDataApi, ICachedReferenceDataApi cachedReferenceDataApi)
    {
        ...
        RuleForEach(x => x.PaymentRequests)
            .NotEmpty()
            .SetValidator((paymentRequest) => new PaymentRequestValidator(_referenceDataApi, _cachedReferenceDataApi, new FieldsRoute(){ AccountType = paymentRequest.AccountType, Organisation = paymentRequest.Organisation, PaymentType = paymentRequest.PaymentType, SchemeType = paymentRequest.SchemeType}, paymentRequest.Status, paymentRequest.AccountType))
