...
{
    public class GetMainAccountCodeAsyncTests
    {
        ...
        public GetMainAccountCodeAsyncTests()
        {
            ...
            _httpContentDeserializerMock.Setup(x => x.DeserializeListAsync<MainAccountCode>(It.IsAny<HttpContent>()))
                .ReturnsAsync(new List<MainAccountCode>()
                {
                new MainAccountCode()
                {
                    Code = "SOS210",
        ...
        public void GetMainAccountCodeAsync_Returns_List()
        {
            _mockReferenceDataRepositoryMock.Setup(x => x.GetMainAccountCodesListAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
                ...
                {
                    Content = new StringContent(JsonSerializer.Serialize(new List<MainAccountCode>()
                    {
                        new MainAccountCode()
                        {
                            Code = "SOS210",
            ...
            response.Data.Should().BeOfType<List<MainAccountCode>>();
            response.Data.Should().HaveCount(2);
            response.Data.Should().BeEquivalentTo(new List<MainAccountCode>()
            {
                new MainAccountCode()
                {
                    Code = "SOS210",
        ...
        [Fact]
        public async Task GetMainAccountCodeAsync_ResponseDataTaskIsFaulted_LogsErrorAndHandlesException()
        {
            // Arrange
            ...
            mockRepository
                .Setup(x => x.GetMainAccountCodesListAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
                .ReturnsAsync(response);

            var service = new ReferenceDataApi(mockRepository.Object, mockLogger.Object, new FaultedHttpContentDeserializer());

            // Act
            var result = await service.GetMainAccountCodesAsync(_accountType, _organisation, _paymentType, _schemeType);
        ...
        [Fact]
        public async Task GetMainAccountCodeAsync_ResponseDataIsNull_ReturnsNotFound()
        {
            // Arrange
            var mockRepository = new Mock<IReferenceDataRepository>();
            var mockLogger = new Mock<ILogger<ReferenceDataApi>>();
